...
export function InvoicesView() {
  ...
  const [emailModal, setEmailModal] = useState<{ invoice: InvoiceFull; email: string; originalEmail: string; customerId: string; saveEmail: boolean; sending: boolean } | null>(null);
  ...
  function openEmailModal(inv: InvoiceFull) {
    ...
    setEmailModal({ invoice: inv, email: existingEmail, originalEmail: existingEmail, customerId: match?.id ?? inv.customerId ?? '', saveEmail: false, sending: false });
  }
  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      notify(`Invoice emailed to ${json.sentTo}${emailModal.saveEmail && isNewEmail ? ' · email saved to customer' : ''}`);
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--card)', borderRadius: 14, padding: 28, width: 440, maxWidth: '95vw', boxShadow: '0 8px 40px rgba(0,0,0,0.35)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <div style={{ fontSize: 17, fontWeight: 800 }}>Email Invoice to Customer</div>
            </div>
            <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 20 }}>
              A PDF copy of <strong>{emailModal.invoice.invoiceNumber}</strong> will be attached.
            ...
            {/* No email on file — offer to save */}
