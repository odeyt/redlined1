...
export function InvoicesView() {
  ...
  const [emailModal, setEmailModal] = useState<{ invoice: InvoiceFull; email: string; originalEmail: string; customerId: string; saveEmail: boolean; sending: boolean; channel: 'email' | 'sms' | 'whatsapp' | 'line' | 'telegram' } | null>(null);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      // Discount is entered directly as a currency amount now; tax stays percentage-based.
      const { amount: effLineTotal } = calcFormEffectiveTotal(form.lines, form.currency);
      const computedDiscount = Math.min(Number(form.discount) || 0, effLineTotal);
  ...
  function openPayModal(inv: InvoiceFull) {
    ...
    setPayModal({ inv, total: amount, currency, mode: 'Cash', cardType: 'Credit', ref: '', saving: false });
  }

  async function handlePayConfirm() {
    if (!payModal) return;
    const { inv, total, currency, mode, cardType, ref } = payModal;
    ...
    try {
      ...
      setSelected(updated);
      const label = mode === 'Card' ? `${cardType} Card ${formatMoney(total, currency)}` : `${mode} ${formatMoney(total, currency)}`;
  ...
  function openEmailModal(inv: InvoiceFull) {
    ...
    setEmailModal({ invoice: inv, email: existingEmail, originalEmail: existingEmail, customerId: match?.id ?? inv.customerId ?? '', saveEmail: false, sending: false, channel: 'email' });
  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      notify(`Invoice emailed to ${json.sentTo}${emailSaved ? ' · email saved to customer record' : ''}`);
  ...
  async function handleSendMessage() {
    ...
    try {
      ...
      notify(`Invoice sent via ${emailModal.channel.toUpperCase()}`);
      setEmailModal(null);
      void t;
    } catch (e: unknown) {
          ...
          {showForm && (
            <form ref={formRef} onSubmit={handleSave} style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 10, padding: 18, marginBottom: 14 }}>
                ...
                {form.lines.map((line, i) => {
                  ...
                  return (
                    <div key={i} style={{ marginBottom: 6 }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 0.5fr 0.9fr 0.75fr 0.8fr 1fr auto', gap: 4 }}>
                        <input value={line.description} onChange={e => setLine(i, 'description', e.target.value)} placeholder="Part / service description"
                          style={{ border: '1px solid var(--line)', borderRadius: 6, padding: '7px 8px', fontSize: 12, background: 'var(--surface)', color: 'var(--text)' }} />
                      ...
                      <input value={line.note} onChange={e => setLine(i, 'note', e.target.value)} placeholder="Note / ref # (optional)"
                        style={{ marginTop: 3, border: '1px solid var(--line)', borderRadius: 4, padding: '3px 8px', fontSize: 10, background: 'var(--surface)', color: 'var(--muted)', width: 'calc(100% - 32px)', boxSizing: 'border-box', display: 'block' }} />
                    </div>
                  );
                })}
              </div>
              ...
              {(() => {
                ...
                const discAmt = Math.min(Number(form.discount) || 0, adjEff);
                ...
                return (
                  <>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
                      <div className="login-field">
                        <label>Discount ({adjCur})</label>
                        <input
                          type="text" inputMode="decimal"
                          value={form.discount === 0 || form.discount === undefined ? '' : String(form.discount)}
                          onChange={e => {
                            const raw = e.target.value.replace(/[^0-9.]/g, '');
                            const n = Math.max(0, parseFloat(raw) || 0);
                            setForm(f => ({ ...f, discount: raw === '' ? 0 : n }));
                          }}
                          placeholder="0"
                      ...
                      <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 10, padding: '12px 16px', marginBottom: 12, fontSize: 13 }}>
                        ...
                        {discAmt > 0 && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', color: '#4caf50', marginBottom: 4 }}>
                            <span>Discount</span><span>-{formatMoney(discAmt, adjCur)}</span>
                          </div>
                        )}
                        {shopSup > 0 && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--muted)', marginBottom: 4 }}>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'var(--card)', borderRadius: 14, padding: 28, width: 460, maxWidth: '95vw', boxShadow: '0 8px 40px rgba(0,0,0,0.35)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                ...
                <div>
                  <div style={{ fontSize: 17, fontWeight: 800 }}>Send Invoice to Customer</div>
                  <div style={{ fontSize: 12, color: 'var(--muted)' }}>{emailModal.invoice.invoiceNumber} · {emailModal.invoice.customerName}</div>
                </div>
              </div>
              ...
              {isEmail && <>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 14 }}>
                  A PDF copy will be emailed with the invoice attached.
              ...
              {!isEmail && <>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 14 }}>
                  A text summary of the invoice details will be sent to the phone number on file for this
                  customer. Configure credentials in <strong>Settings → Messaging</strong>. If no phone
                  number is on file, add one to the customer record first.
                </div>
              </>}
          ...
          <div style={{ background: 'var(--card)', borderRadius: 16, width: '100%', maxWidth: 460, boxShadow: '0 16px 60px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 18 }}>
                {(['Cash', 'Card', 'QR Code'] as const).map(m => (
                  <button key={m} type="button"
                    onClick={() => setPayModal(pm => pm ? { ...pm, mode: m } : null)}
                    ...
                    {m === 'Cash' ? '💵 Cash' : m === 'Card' ? '💳 Card' : '📱 QR Code'}
                  </button>
                ))}
              </div>
              ...
              {payModal.mode === 'Card' && (
                <>
                  <div style={{ marginBottom: 14 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Card Type</label>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      {(['Credit', 'Debit'] as const).map(ct => (
                        <button key={ct} type="button"
                          onClick={() => setPayModal(pm => pm ? { ...pm, cardType: ct } : null)}
                          style={{ padding: '9px 6px', borderRadius: 8, border: payModal.cardType === ct ? '2px solid var(--accent)' : '1px solid var(--line)', background: payModal.cardType === ct ? 'rgba(204,0,0,0.07)' : 'var(--surface)', color: payModal.cardType === ct ? 'var(--accent)' : 'var(--text)', fontWeight: payModal.cardType === ct ? 800 : 600, fontSize: 13, cursor: 'pointer' }}>
                          💳 {ct}
                        </button>
                      ))}
                    </div>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Card Reference / Last 4 Digits (optional)</label>
                    <input value={payModal.ref} onChange={e => setPayModal(pm => pm ? { ...pm, ref: e.target.value } : null)}
                      placeholder="e.g. Auth #123456 or •••• 4242"
