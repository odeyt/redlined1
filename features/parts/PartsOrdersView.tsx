...
const EMPTY_ORDER: Omit<PartsOrder, 'id' | 'createdAt'> = {
...
export function PartsOrdersView() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [o, v] = await Promise.all([fetchPartsOrders(), fetchVendors()]);
      setOrders(o); setVendors(v);
    } catch (e: unknown) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        const updated = await updatePartsOrder(editingId, form);
        setOrders(prev => prev.map(o => o.id === editingId ? updated : o));
        notify(`${updated.partName} updated.`);
      } else {
        const created = await createPartsOrder(form);
        setOrders(prev => [created, ...prev]);
        notify(`${created.partName} order added.`);
      }
      setShowForm(false); setEditingId(null); setForm(EMPTY_ORDER);
    } catch (e: unknown) {
      notify((e as Record<string,unknown>)?.message as string || 'Save failed');
  ...
  /* vendor select auto-fills phone/email */
  ...
  /* summary stats */
  const totalOrdered  = orders.filter(o => o.status === 'Ordered' || o.status === 'Deposit Paid' || o.status === 'Backordered').length;
  ...
  const sel = (val: string, cb: (v: string) => void, opts: string[]) => (
    <select value={val} onChange={e => cb(e.target.value)}
      style={{ border: '1px solid var(--line)', borderRadius: 8, padding: '10px 12px', background: 'var(--surface-soft)', width: '100%' }}>
  ...
  return (
    <div style={{ padding: '20px 24px' }}>
      ...
      {error && <div style={{ marginBottom: 12, padding: '10px 14px', background: 'rgba(239,68,68,.1)', color: '#ef4444', borderRadius: 8, fontSize: 13 }}>{error} <button onClick={() => setError('')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }}>✕</button></div>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 24 }}>
        {[
          ...
          { label: 'Balance Due', value: money(totalOwed),   color: '#ef4444', sub: 'outstanding to vendors' },
          { label: 'Deposits Paid', value: money(totalDeposits), color: '#8b5cf6', sub: 'across all orders' },
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search part, vendor, job card, vehicle…"
          ...
          : (
            <div style={{ overflowX: 'auto' }}>
              <table>
                <thead>
                  <tr>
                    <th>Part</th><th>Vendor</th><th>Qty</th><th>Unit Cost</th>
                    <th>Deposit</th><th>Balance</th><th>Status</th><th>Payment</th>
                    <th>ETR</th><th>Linked To</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {visible.map(o => (
                    <tr key={o.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(o)}>
                      ...
                      <td style={{ color: o.depositPaid > 0 ? '#8b5cf6' : 'var(--muted)' }}>{money(o.depositPaid)}</td>
                      <td style={{ fontWeight: 700, color: o.balanceDue > 0 ? '#ef4444' : '#22c55e' }}>{money(o.balanceDue)}</td>
                      ...
                      <td style={{ fontSize: 11 }}>
                        {o.jobCardNumber && <div>JC: {o.jobCardNumber}</div>}
                        {o.repairOrderNumber && <div>RO: {o.repairOrderNumber}</div>}
                        {o.estimateNumber && <div>EST: {o.estimateNumber}</div>}
                        {o.invoiceNumber && <div>INV: {o.invoiceNumber}</div>}
                        {o.vehicle && <div style={{ color: 'var(--muted)' }}>{o.vehicle}</div>}
                      </td>
                      <td onClick={e => e.stopPropagation()}>
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 480, maxWidth: '95vw', background: 'var(--bg)', borderLeft: '1px solid var(--line)', zIndex: 301, overflowY: 'auto', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ padding: '20px 24px', flex: 1 }}>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Pricing</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[
                    { label: 'Unit Cost', value: money(selected.unitCost) },
                    { label: 'Qty × Total', value: `${selected.quantity} × ${money(selected.unitCost)} = ${money(selected.totalCost)}` },
                    { label: 'Core Charge', value: money(selected.coreCharge) },
                    { label: 'Deposit Paid', value: money(selected.depositPaid), color: '#8b5cf6' },
                    { label: 'Balance Due', value: money(selected.balanceDue), color: selected.balanceDue > 0 ? '#ef4444' : '#22c55e' },
              ...
              {selected.vendorName && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Vendor</div>
                  <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '12px 14px' }}>
              ...
              {/* Dates */}
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Dates</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                  {[
                    ...
                    <div key={label} style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 12px' }}>
                      <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 3 }}>{label}</div>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{value ? new Date(value).toLocaleDateString() : '—'}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Links */}
              {(selected.jobCardNumber || selected.repairOrderNumber || selected.estimateNumber || selected.invoiceNumber || selected.vehicle || selected.customerName) && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Linked Records</div>
                  <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {selected.customerName && <div style={{ fontSize: 13 }}>👤 {selected.customerName}</div>}
              ...
              {(selected.warranty || selected.notes) && (
                <div style={{ marginBottom: 20 }}>
                  {selected.warranty && (
                    <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '12px 14px', marginBottom: 8 }}>
                      <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>Warranty</div>
                      <div style={{ fontSize: 13 }}>{selected.warranty}</div>
                    </div>
                  )}
                  {selected.notes && (
                    <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '12px 14px' }}>
                      <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>Notes</div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '24px 16px', overflowY: 'auto' }}
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 680, boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 20 }}>{editingId ? 'Edit Parts Order' : 'New Parts Order'}</div>

            <form onSubmit={handleSave}>
              {/* Part Info */}
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Part Details</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                ...
                {field('Condition', sel(form.condition, v => setF({ condition: v }), PART_CONDITIONS))}
                ...
                <div className="login-field">
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <select value={form.vendorName} onChange={e => handleVendorSelect(e.target.value)}
                      style={{ flex: 1, border: '1px solid var(--line)', borderRadius: 8, padding: '10px 12px', background: 'var(--surface-soft)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 12 }}>
                {field('Unit Cost ($)', <input type="number" min={0} step="0.01" value={form.unitCost} onChange={e => setF({ unitCost: Number(e.target.value) || 0 })} />)}
                {field('Core Charge ($)', <input type="number" min={0} step="0.01" value={form.coreCharge} onChange={e => setF({ coreCharge: Number(e.target.value) || 0 })} />)}
                {field('Deposit Paid ($)', <input type="number" min={0} step="0.01" value={form.depositPaid} onChange={e => setF({ depositPaid: Number(e.target.value) || 0 })} />)}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
                <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 3 }}>Total Cost</div>
                  <div style={{ fontWeight: 800, fontSize: 16 }}>{money(form.totalCost)}</div>
                </div>
                <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 3 }}>Balance Due</div>
                  <div style={{ fontWeight: 800, fontSize: 16, color: form.balanceDue > 0 ? '#ef4444' : '#22c55e' }}>{money(form.balanceDue)}</div>
                </div>
                <div style={{ background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 14px' }}>
                  <div style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 3 }}>Payment</div>
                  <div style={{ fontWeight: 700, fontSize: 13, color: PAY_COLOR[form.paymentStatus] || '#888' }}>{form.paymentStatus}</div>
                </div>
              </div>

              {/* Status & Dates */}
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Status & Dates</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                {field('Order Status', sel(form.status, v => setF({ status: v }), ORDER_STATUSES))}
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>Linked Records</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
                {field('Customer Name', inp('text', form.customerName, v => setF({ customerName: v }), 'e.g. John Smith'))}
                {field('Vehicle', inp('text', form.vehicle, v => setF({ vehicle: v }), 'e.g. 2020 Toyota Camry'))}
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving…' : editingId ? 'Update Order' : 'Save Order'}</button>
              </div>
            </form>
          </div>
        </div>
