        ...
        <div onClick={() => { setShowForm(false); setEditingId(null); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '24px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 16, padding: 28, width: '100%', maxWidth: 800, boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              {/* ── Photos & Invoices ── */}
              {editingId ? (
                <div style={{ marginBottom: 24, padding: '16px 18px', background: 'var(--surface-soft)', border: '1px solid var(--line)', borderRadius: 12 }}>
                  <FormSection label="📎 Photos & Invoices" />
                  <div style={{ display: 'flex', gap: 8, marginBottom: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                    <select value={imgLabel} onChange={e => setImgLabel(e.target.value as 'Photo' | 'Invoice')}
                      style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--line)', background: 'var(--card)', fontSize: 13 }}>
                      <option value="Photo">📷 Photo</option>
                      <option value="Invoice">🧾 Invoice</option>
                    </select>
                  ...
                  <label
                    style={{ display: 'block', border: '2px dashed var(--line)', borderRadius: 10, padding: '16px', textAlign: 'center', cursor: 'pointer', marginBottom: 10, background: 'var(--card)', transition: 'border-color .15s' }}
                ...
                <div style={{ marginBottom: 24, padding: '10px 14px', background: 'var(--surface-soft)', borderRadius: 8, border: '1px solid var(--line)', fontSize: 12, color: 'var(--muted)' }}>
                  📎 <strong>Photos & Invoices</strong> — save this order first, then re-open it to attach vendor invoices and photos.
                </div>
              )}
