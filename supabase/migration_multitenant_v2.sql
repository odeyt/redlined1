-- ═══════════════════════════════════════════════════════════════
-- MULTI-SHOP MIGRATION v2 — Full reset, defensive, runs clean
-- New query in Supabase SQL Editor — replaces all previous attempts
-- ═══════════════════════════════════════════════════════════════

-- ── 1. Shops table (safe to re-run) ──────────────────────────────
...
-- ── 3. Add shop_id column to every data table (IF NOT EXISTS) ─────
...
-- ── 4. Backfill all existing rows → D1 Imports (shop 1) ──────────
DO $$
DECLARE shop1_id UUID;
BEGIN
...
-- ── 5. Insert Location 2 shop_settings row ───────────────────────
...
-- ── 9. Verify ─────────────────────────────────────────────────────
SELECT 'Shops created:'        AS step, COUNT(*)::text AS result FROM public.shops;
SELECT 'Shop users linked:'    AS step, COUNT(*)::text AS result FROM public.shop_users;
SELECT 'Customers backfilled:' AS step, COUNT(*)::text AS result FROM public.customers  WHERE shop_id IS NOT NULL;
SELECT 'Invoices backfilled:'  AS step, COUNT(*)::text AS result FROM public.invoices   WHERE shop_id IS NOT NULL;
SELECT 'Payments backfilled:'  AS step, COUNT(*)::text AS result FROM public.payments   WHERE shop_id IS NOT NULL;
SELECT 'Shop settings rows:'   AS step, COUNT(*)::text AS result FROM public.shop_settings;
SELECT 'D1 Imports shop_id:'   AS step, id::text       AS result FROM public.shops WHERE slug = 'd1-imports';
SELECT 'Location 2 shop_id:'   AS step, id::text       AS result FROM public.shops WHERE slug = 'd1-imports-location-2';
