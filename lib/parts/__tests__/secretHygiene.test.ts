  ...
  it('no fixture, test or doc carries a plausible provider key', () => {
    ...
    for (const f of FILES) {
      ...
      for (const line of src.split(/\r?\n/)) {
        ...
        // A kebab- or snake-case slug is an identifier — an endpoint path or a
        // table name — not a credential. The provider's own paths run past 60
        // characters. A real key carries mixed case rather than lowercase
        // words joined by separators.
        if (/^[a-z0-9]+([_-][a-z0-9]+)+$/.test(m[1])) continue;
