...
export const initialState: AppState = {
  ...
  dtcResult: null,
  diagnosticCodes: [],
  toast: 'Ready. Use the workflow buttons to create jobs, approve work, invoice, and save diagnostics. QA sign-off is now required before closing any Repair Order.',
  activityLog: ['System ready', 'AI Copilot loaded', 'QA sign-off enforcement active — ROs require advisor approval to close', 'Return Job feature added to Archive, Repair Orders, Vehicle Drawer & Job Cards'],
  users: initialUsers,
...
export function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'EDIT_APPOINTMENT':
      return { ...state, appointments: state.appointments.map((a, i) => i === action.appointmentIndex ? action.appointment : a), ...notify(state, `Appointment updated for ${action.appointment[1]}.`) };
