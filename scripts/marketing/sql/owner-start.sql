...
checks (ord, section, check_name, expected, actual, verdict) AS (
  ...
  FROM trig_pin
  UNION ALL
  SELECT fv.ord, 'C live definitions', 'md5(prosrc) of public.' || fv.fname || ' equals the production definition measured 2026-09-16',
  ...
  SELECT 61, 'C live definitions', 'walkthrough alerts proven from those definitions',
    ...
    CASE WHEN (SELECT expected = actual FROM trig_pin) AND NOT EXISTS (SELECT 1 FROM fn_verdict WHERE verdict <> 'PASS')
         THEN 'proven: live triggers and functions equal the pinned definitions' ELSE 'not proven: see rows 54-60' END,
    CASE WHEN (SELECT expected = actual FROM trig_pin) AND NOT EXISTS (SELECT 1 FROM fn_verdict WHERE verdict <> 'PASS')
         THEN 'PASS' ELSE 'STOP' END
